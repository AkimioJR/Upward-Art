...
1. **案例库**：在表格末尾追加新案例行，格式与现有行一致（序号、案例链接、一句话概括）。
2. **你遇到的问题 → 参考案例**：如果新案例属于表中已有的问题场景，在对应行追加案例编号；如果新案例属于新的问题场景，新增一行。

### 案例索引更新

每次归档新案例后，同步更新 `references/case-index.md`：

1. **场景速查表**：如果新案例属于表中已有的场景分类，在对应行追加案例编号；如果属于新的场景分类，新增一行。
2. **案例详情**：在文件末尾新增一条案例条目，格式如下：
